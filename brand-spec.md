# Brand Specification: Auren (Linear Style)
...
**Typography**
- Display: Inter Tight at weight 600
- Body: Inter at 14–15px, weight 400–500, line-height 1.55
- Mono: GeistMono or JetBrains Mono for inline code and shortcut chips
- Letter-spacing on display headlines: -0.02em (tight)

**Spacing**: 4 / 8 / 12 / 16 / 24 / 40 / 64 / 96

**Radius**: 6 (small UI) / 12 (medium cards) / 16 (large panels). **Never above 16.**

**Shadow**: barely there — soft `0 1px 2px rgba(0,0,0,0.3)` on raised surfaces. **Never glow, never colored shadow.**

**Motion**: ease-out around 150ms for hover, 350–450ms for layout moves with a quint curve. State changes feel "snappy but not bouncy."
...
- Keyboard-shortcut chips throughout the UI
- Subtle gradient meshes in the hero (extremely controlled saturation, < 8% opacity)
